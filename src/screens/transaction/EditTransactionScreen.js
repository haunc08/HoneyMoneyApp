...
import { sizeFactor, styles, colors } from "../../constants";
...
export class EditTransactionScreen extends Component {
  ...
  componentDidMount() {
    ...
    this.props.chooseCategory(
      categories.filter((item) => item.key == trans.category.key)[0]
    );
  ...
  editTransaction = () => {
    ...
    const tempmoney =
      this.props.selectedWallet.transactionList[this.props.selectedTransaction]
        .money;
    ...
    if (b) {
      ...
      userWalletRef.child(this.props.selectedWallet.key).update({
        money:
          this.props.selectedWallet.money -
          this.props.newSoDu +
          parseInt(tempmoney),
      });
    }
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        <Calculator />
        <ScreenView
          style={{ backgroundColor: this.props.selectedWallet?.color, flex: 1 }}
        >
          <View
            style={{
              alignItems: "flex-end",
